...
int main(int argc, char **argv)
{
  ...
  cv::Mat x_k(2, 1, CV_64F);

  // Initialize state with a random state
  //randn(x_k, 0.0, 0.1);

  // Initialize state with known state
  x_k.at<double>(0) = 0.0;
  x_k.at<double>(1) = 0.0;

  // Control is 1x1 (c=1) [angular acceleration]
  cv::Mat u_k(1, 1, CV_64F);

  // Init w/ zero acceleration
  u_k.at<double>(0) = 0;
